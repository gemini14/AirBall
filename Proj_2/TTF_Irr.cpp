...
{
	FT_Set_Pixel_Sizes(ttFace_.face, 0, size);
	if ( size > largestSize.Height )
		largestSize.Height = size;

	if ( !FT_Load_Glyph(ttFace_.face, idx_, FT_LOAD_NO_HINTING|FT_LOAD_NO_BITMAP) )
	{
		FT_GlyphSlot glyph = ttFace_.face->glyph;
		FT_Bitmap  bits;
		...
		if (glyph->format == ft_glyph_format_outline )
		{
			if (!FT_Render_Glyph( glyph, FT_RENDER_MODE_NORMAL))
			{
				...
				s32 offx = left;
				s32 offy = size - top;
				if ( offx+texw > largestSize.Width )
					largestSize.Width = offx+texw;
				if ( offy+texh > largestSize.Height )
					largestSize.Height = offy+texh;
				...
				setGlyphTextureFlags(driver_);
				tex = driver_->addTexture(name,img);
				img->drop();
				restoreTextureFlags(driver_);
				delete[] texd;
				cached = true;
			}
		}
	...
	if (!FT_Load_Glyph(ttFace_.face,idx_,FT_LOAD_NO_HINTING|FT_LOAD_RENDER|FT_LOAD_MONOCHROME))
	{
		FT_GlyphSlot glyph = ttFace_.face->glyph;
...
u32 CGUIFreetypeFont::getGlyphByChar(wchar_t c) const
{
	...
	if ( idx && !Glyphs[idx - 1]->cached )
        Glyphs[idx - 1]->cache(idx, *TrueTypeFace, Driver, LargestGlyph);
	return	idx;
}
